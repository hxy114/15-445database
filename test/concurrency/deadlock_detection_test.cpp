...
TEST(LockManagerDeadlockDetectionTest, BasicDeadlockDetectionTest) {
  ...
  std::thread t0([&] {
    ...
    std::cout << "t0 lock table intention exclusive" << std::endl;
    EXPECT_EQ(true, res);
    res = lock_mgr.LockRow(txn0, LockManager::LockMode::EXCLUSIVE, toid, rid0);
    std::cout << "t0 lock  row exclusive0" << std::endl;
    EXPECT_EQ(true, res);
    ...
    std::cout << "t0 lock  row exclusive1" << std::endl;
    EXPECT_EQ(true, res);

    lock_mgr.UnlockRow(txn0, toid, rid1);
    std::cout << "t0 unlock" << std::endl;
  ...
  std::thread t1([&] {
    ...
    std::cout << "t1 lock table intention exclusive" << std::endl;
    EXPECT_EQ(res, true);

    res = lock_mgr.LockRow(txn1, LockManager::LockMode::EXCLUSIVE, toid, rid1);
    std::cout << "t1 lock  row exclusive0" << std::endl;
    EXPECT_EQ(TransactionState::GROWING, txn1->GetState());

    // This will block
    res = lock_mgr.LockRow(txn1, LockManager::LockMode::EXCLUSIVE, toid, rid0);
    std::cout << "t1 lock  row exclusive1" << std::endl;
    EXPECT_EQ(res, false);

    EXPECT_EQ(TransactionState::ABORTED, txn1->GetState());
    std::cout << "t1 abort0" << std::endl;
    txn_mgr.Abort(txn1);
    std::cout << "t1 abort1" << std::endl;
  });
