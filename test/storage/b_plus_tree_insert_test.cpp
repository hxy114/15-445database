...
TEST(BPlusTreeTests, InsertTest2) {
  ...
  std::vector<int64_t> keys = {0, 4, 3, 2, 1};
  for (auto key : keys) {
    int64_t value = key & 0xFFFFFFFF;
...
TEST(BPlusTreeTests, InsertTest3) {
  ...
  for (auto iterator = tree.Begin(); iterator != tree.End(); ++iterator) {
    ...
    EXPECT_EQ((*iterator).first.ToString(), keys[current_key - 1]);
    EXPECT_EQ(location.GetSlotNum(), keys[current_key - 1]);
    /*std::cout << "current_key  " << current_key << std::endl;
    std::cout << "keys  " << keys[current_key - 1] << std::endl;
    std::cout << " location.GetSlotNum() " << location.GetSlotNum() << std::endl;*/
